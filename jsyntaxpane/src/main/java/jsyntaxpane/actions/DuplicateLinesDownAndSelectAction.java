 ...
 * below the current selection. On completion of the action, the inserted lines
 * are selected.
 * 
 * @author Gabriel Hauber (gabriel@ghostdust.com)
 */
public class DuplicateLinesDownAndSelectAction extends DefaultSyntaxAction {

    public DuplicateLinesDownAndSelectAction() {
        super("DUPLICATE_LINES_DOWN");
    }
    ...
    @Override public void actionPerformed(JTextComponent textComponent, SyntaxDocument syntaxDocument, int dot, ActionEvent event) {
        try {
            ...
            String content = syntaxDocument.getText(start, length);
            
            // to insert *before* the current selection, we can insert the new text before, and select
            // what was the original text
            syntaxDocument.insertString(start, content, null);
            
            // select the inserted text, excluding the final carriage-return
            textComponent.select(start + length, end + length - 1);
